...
BDI frameworks are usually described in terms of an \textit{agent theory} and an \textit{agent computational architecture} \cite{Fisher2007}. % The latter could be further divided into data structure and execution cycle. 
...
% and that puts scalability among the requirements it aims to satisfy. 
...
%The steps of a plan body can include belief query, belief update, sub-goal adoption, primitive actions, variable assignment, and control flow structures (loops and conditionals). 
...
%From a functional perspective, ASC2 agents (by default) conceptually follow a reasoning flow very close to the reasoning cycle defined by AgentSpeak(L)/Jason. 
...
The second type are terms that can be calculated locally, and for example are used in control flow structures \asc{if/else} or variable assignments. As an example, if in the context of execution of a plan we have a variable \asc{Loc} that is grounded by the string value \asc{"french"}, the statement:
\begin{minted}[fontsize=\small]{prolog}
...
At syntactic level, initial beliefs and inferential rules are logic-style terms, and as such they translate to an \asc{Term} object counterpart. Initial goals are a combination of a prefix (\asc{!}, designating the adoption of a new goal)
...
The plan body $h$ of a plan rule consist of zero or more steps. %and each step can be one of the plan step types. 
%The plan body 
